...
class FeatureExtractor:
    """病历特征提取器 - 使用 HashingVectorizer 支持任意文本"""

    # 固定维度，2^18 = 262144
    HASH_DIM = 2 ** 18

    def __init__(self):
        # HashingVectorizer - 支持任意文本，无需训练
        self.text_vectorizer = HashingVectorizer(
            n_features=self.HASH_DIM,
            norm='l2',  # L2 归一化
            alternate_sign=False  # 所有值非负，便于相似度计算
        )
    ...
    def total_dim(self) -> int:
        """总特征维度 = 文本哈希维度 + 检验维度 + 药物维度 + 人口学维度"""
        return self.HASH_DIM + self.lab_dim + self.med_dim + self.demo_dim

    def fit(self, records: List[MedicalRecord]):
        """HashingVectorizer 不需要训练"""
        pass
    ...
    def extract(self, record: MedicalRecord) -> np.ndarray:
        ...
        Returns:
            合并后的特征向量
        """
        # 1. 文本向量 (HashingVectorizer)
        text_vec = self._extract_text_vector(record)
        ...
        # 5. 合并所有向量
        combined = np.concatenate([text_vec, lab_vec, med_vec, demo_vec])

        return combined

    def _extract_text_vector(self, record: MedicalRecord) -> np.ndarray:
        """
        用 HashingVectorizer 提取文本向量
        将诊断、检验类别、药物名称合并成一段文本
        """
        # 提取诊断文本
        diag_text = ' '.join([d.name for d in record.diagnoses])

        # 提取检验类别文本
        lab_text = ' '.join(record.lab_results.keys())

        # 提取药物文本
        med_text = ' '.join([m.name for m in record.medications])

        # 合并所有文本
        full_text = f"{diag_text} {lab_text} {med_text}"

        # 向量化
        vec = self.text_vectorizer.transform([full_text]).toarray()[0]
